# This code carries out the WRTDS bootstrap test (WBT)
...
colnames(bootOut) <- c("rejectC","pValC","estC","lowC","upC","lowC50","upC50","lowC95","upC95","likeCUp","likeCDown","rejectF","pValF","estF","lowF","upF","lowF50","upF50","lowF95","upF95","likeFUp","likeFDown","baseConc","baseFlux","iBoot")
...
#	cat("\niBoot,xConc[1],xConc[iBoot]",iBoot,xConc[1],xConc[iBoot])			
			...
			rejectC <- if(lowConc * highConc > 0) TRUE else FALSE
			rejectF <- if(lowFlux * highFlux > 0) TRUE else FALSE
...
cat("\n\n",format(wordsOut[1],width=30),format(wordsOut[3],width=30))
cat("\n",format(wordsOut[2],width=30),format(wordsOut[4],width=30))
xConc <- xConc[1:iBoot]
xFlux <- xFlux[1:iBoot]
...
wordLike <- function(likeList){
	...
	secondPart <- c("extremely unlikely","very unlikely","unlikely","about as likely as not","likely","very likely","extremely likely")
