...
\author[1]{Stacey A. Archfield}
\author[1]{Laura A. De Cicco}
\affil[1]{United States Geological Survey}

\noindent{\huge\textsf{\textbf{Introduction to the EGRETci package}}}

\noindent\textsf{By Robert M. Hirsch, Stacey A. Archfield, and Laura A. De Cicco}
...
%------------------------------------------------------------ 
    ...
    This package EGRETci is an early test version of the package that will encompass the various approaches to uncertainty estimation associated with trend analysis for the EGRET package.  The various functions included here are all discussed in the draft manuscript `\textit{A bootstrap method for estimating uncertainty of water quality trends}' by Hirsch, Archfield, and De Cicco (draft version of March 5, 2015).  The present version of the EGRETci package is designed for interactive use only, but later versions will describe how batch workflows can be created.  Also, the current version can only do analysis of trends for water years, but later versions will allow it to evaluate any group of months (i.e. any `Period of Analysis' to use the term in the EGRET package).  The EGRETci package is designed to carry out three specific types of tasks.
    
    
1)  Evaluate a trend over a specific span of years and produce a variety of tabular results.  This is done with a short workflow involving four functions: \texttt{trendSetUp}, \texttt{setPA}, \texttt{setForBoot}, and \texttt{wBT}.  The results come both in the form of a) console output, which shows the bootstrap replicate process as it is underway and provides text output of results, and b) a list of outputs called eBoot.  The contents of eBoot are described below.


2)	Prepare histograms of values for the trend magnitudes, expressed in percent, for flow-normalized concentration and flow-normalized flux.  This is done with the function \texttt{plotHistogramTrend}.  It depends on outputs contained in eBoot.


3)	Create confidence bands around the computed trends in flow-normalized concentration and flow-normalized flux.  This is done using a single interactive function called xxxxx and then, using the output from that function running two functions that produce the confidence band graphics for concentration and flux respectively (\texttt{plotConcHistBoot}, and \texttt{plotFluxHistBoot}).


\begin{table}[!ht]
{\footnotesize
\caption{This table defines the contents of the eBoot list} 
\label{table:eBoot1}
\begin{tabularx}{\textwidth}{llX}
  \hline
\multicolumn{1}{c}{\textbf{\textsf{Data Frame}}} &
\multicolumn{1}{c}{\textbf{\textsf{Column}}} &
\multicolumn{1}{c}{\textbf{\textsf{Definition}}} \\ 
  \hline
  bootOut & rejectC & Reject Ho, (no trend in concentration), TRUE or FALSE \\ 
  [5pt] & pValC & p-value for no trend in concentration \\ 
  [5pt] & estC & standard WRTDS estimate of change from starting year to ending year in mg/L \\ 
  [5pt] & lowC & Lower confidence limit (90\%) on concentration trend \\ 
  [5pt] & upC & Upper confidence limit (90\%) on concentration trend \\ 
  [5pt] & lowC50 & Lower confidence limit (50\%) on concentration trend \\ 
  [5pt] & upC50 & Upper confidence limit (50\%) on concentration trend \\
  [5pt] & lowC95 & Lower confidence limit (95\%) on concentration trend \\ 
  [5pt] & upC95 & Upper confidence limit (95\%) on concentration trend \\ 
  [5pt] & likeCUp & Likelihood that trend in concentration is upwards \\ 
  [5pt] & likeCDown & Likelihood that trend in concentration is downwards \\
  [5pt] & rejectF & Reject Ho, (no trend in flux), TRUE or FALSE \\ 
  [5pt] & pValF & p-value for no trend in flux \\
  [5pt] & estF & standard WRTDS estimate of change from starting year to ending year, in 10\textsuperscript{6} kg/yr \\
  [5pt] & lowF & Lower confidence limit (90\%) on flux trend \\
  [5pt] & upF & Upper confidence limit (90\%) on flux trend \\
  [5pt] & lowF50 & Lower confidence limit (50\%) on flux trend \\
  [5pt] & upF50 & Upper confidence limit (50\%) on flux trend \\
  [5pt] & lowF95 & Lower confidence limit (95\%) on flux trend \\
  [5pt] & upF95 & Upper confidence limit (95\%) on flux trend \\
  [5pt] & likeFUp & Likelihood that trend in flux is upwards \\
  [5pt] & likeFDown & Likelihood that trend in flux is downwards \\
  [5pt] & baseConc & The estimated mean flow-normalized concentration in starting year, in mg/L \\
  [5pt] & baseFlux & The estimated mean flow-normalized flux in starting year, in 10\textsuperscript{6} kg/yr \\
  [5pt] & iBoot & The actual number of bootstrap replicates used \\
  [5pt]wordsOut & & a vector of four character variables (self explanatory) \\
  [5pt]xConc & & a vector of length iBoot, of the change in flow normalized concentration computed by each bootstrap replicate (mg/L) \\
  [5pt]xFlux & & a vector of length iBoot, of the change in flow normalized flux computed by each bootstrap replicate (10\textsuperscript{6} kg/yr) \\
   \hline
\end{tabularx}
}
\end{table}
...
There is also a data frame called \texttt{caseSetUp}, which contains a number of important parameters that define the way that the test was implemented.  They are presented here.

\begin{table}[!ht]
{\footnotesize

\caption{This table defines the contents of the \texttt{caseSetUp} data frame} 
\label{table:caseSetUp}
...
\multicolumn{1}{c}{\textbf{\textsf{Definition}}} \\ 
  ...
  [5pt]yearData1 & the water year that is the start of the data set (an integer) \\ 
  [5pt]year2 & the water year that is the end of the trend period (an integer) \\
  [5pt]yearData2 & the water year that is the end of the data set (an integer) \\
  [5pt]numSamples & number of samples in eList\$Sample \\
  [5pt]nBoot & maximum number of replicates (called Mmax in paper) \\
  [5pt]bootBreak & minimum number of replicates (called Mmin in paper) \\
  [5pt]blockLength & length of blocks for bootstrap (called B in the paper) \\
  [5pt]confStop & 1 - alphap, the width of the confidence interval used in adaptive stopping rule (default alphap=0.3 so confStop=0.7) \\
   \hline
\end{tabularx}
...
%------------------------------------------------------------ 
These functions plot histograms of all of the trend slopes from the full set of replicates created by wBT.  These slopes are stored in the eBoot list.  They can be plotted as an individual histogram for trends in Flow Normalized Concentration and Flow Normalized Flux, or they can be combined using the script shown below in Figure \ref{fig:histExampleCombo}:
