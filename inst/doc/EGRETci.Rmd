...
The following is an example workflow for an interactive run of the WBT (WRTDS Bootstrap Test), using the example data set in Choptank_eList.  Note that if one wishes to use smoothing parameters other than the default values, they need to be set within eList before running the `wBT` function. For example, if one wanted to use windowY = 10 (rather than the 7 year window that is the default), the eList being used would have to be modified like this:  eList$INFO$windowY <- 10.
...
caseSetUp <- trendSetUp(eList, year1=1990, year2=2012, nBoot = 50, bootBeak = 39, blockLength = 200, paStart = 10, paLong = 12)
...
Should we reject Ho that Flow Normalized Concentration Trend = 0 ? Reject Ho
 best estimate is   0.328 mg/L
  Lower and Upper 90% CIs    0.260    0.408
 also 95% CIs    0.251    0.412 
 and 50% CIs    0.308    0.349
...
Should we reject Ho that Flow Normalized Flux Trend = 0 ? Reject Ho
 best estimate is  0.03148 10^6 kg/year
  Lower and Upper 90% CIs   0.0235   0.0408
 also 95% CIs   0.0221   0.0451 
 and 50% CIs   0.0283   0.0353
